WITH bronze_layer AS (
	...
	,act.closed
	,act.status_id
	,act.is_running_since
	,act.is_running_at
	,act.next_run
	,act.precedent_activity_id
	,act.precedent_outcome
	,act.due_date
	,act.expired
	,act.skipped
	,act.error_count
	...
	,act.last_error_id
	,act.last_modified
	,act.bronze_created_on
	,act.bronze_updated_on
	,def.id AS activity_definition_id
	,def.ss_key
	,def.name AS activity_definition_name
	,def.description
	,def.kind
	,def.process_def_id
	,def.is_active
	,def.display_x
	,def.display_y
	,def.invoked_process_def_id
	,def.requires_permission
	,def.inbox_instructions
	,def.skippable
    FROM dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".ossys_BPM_Activity act
    LEFT JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".ossys_BPM_Activity_Definition def 
        ON def.id = act.activity_def_id
),
