WITH bronze_layer AS (
    SELECT
        ...
        error_count,
        next_run
    FROM dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".ossys_BPM_Event_Queue
),
...
silver_layer AS (
    SELECT 
        ...
        error_count,
        next_run
    FROM dev_iceberg."tmkn-aws-dwh-dev-iceberg-silver".BPM_EVENT_QUEUE_BASE
)
...
FROM (
    ...
    WHERE
     ...
     OR COALESCE(CAST(b.data_id AS VARCHAR), '') <> COALESCE(CAST(s.data_id AS VARCHAR), '')
