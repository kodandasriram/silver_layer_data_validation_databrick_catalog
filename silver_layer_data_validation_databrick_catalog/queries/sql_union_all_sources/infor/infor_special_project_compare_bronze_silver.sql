-- Compare bronze-layer Infor special project contracts with silver-layer special_projects_agreement_base.
...
silver_layer AS (
    SELECT
        ...
        dbt_updated_at,
        source_system_name
    FROM dev_iceberg."tmkn-aws-dwh-dev-iceberg-silver".special_projects_agreement_base
),
...
bronze_normalized AS (
    SELECT
        ...
        CAST(contract_description AS VARCHAR) AS contract_description,
        ...
        CAST(vendor_reference AS VARCHAR) AS vendor_reference,
        CAST(accounting_unit_code AS VARCHAR) AS accounting_unit_code,
        CAST(accounting_unit_description AS VARCHAR) AS accounting_unit_description,
        CAST(is_on_hold AS VARCHAR) AS is_on_hold,
        ...
        CAST(maximum_amount AS VARCHAR) AS maximum_amount,
        CAST(amount_ordered AS VARCHAR) AS amount_ordered,
        CAST(commitment_amount_remaining AS VARCHAR) AS commitment_amount_remaining,
        CAST(performance_bond_pct AS VARCHAR) AS performance_bond_pct,
