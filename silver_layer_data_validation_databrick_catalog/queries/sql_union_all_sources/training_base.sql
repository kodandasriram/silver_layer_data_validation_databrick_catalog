...
--
-- Bronze source: C:\Users\MODICHERLA\OneDrive - Hexalytics, Inc\Documents\Requirements\Silver Layer\Union All sources\updated_silver_layer_scripts\union of os2_os1_mis\training_base_os2_os1_mis_union_bronze_layer.sql
-- Silver source: C:\Users\MODICHERLA\OneDrive - Hexalytics, Inc\Documents\Requirements\Silver Layer\Union All sources\updated_silver_layer_scripts\silver_layer_query\training_base_silver_layer.sql
...
Description : Unified Silver model for OS2 training-related application supports.
              ...
              The two are NOT 1:1 — see TRAINING_UNIFIED_BASE for the cross-system view.
...
WITH final_base AS (
    ...
    SELECT
        ...
        CAST(NULL AS VARCHAR)                                               AS approval_letter_accepted_on,
        ...
        CAST(NULL AS VARCHAR)                                               AS effective_tamkeen_share_amount_bhd
    ...
    LEFT JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".OSUSR_VW9_TRAININGPAYEE payee
    ...
    INNER JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".OSUSR_NTP_AMENDMENTREQUEST amendreq
...
SELECT
    ...
    -- Identifiers
    CAST(trn.tws_trainingenrollmentid AS VARCHAR)        AS training_enrollment_id,
    trn.tws_name                                         AS training_enrollment_name,
    CAST(trn.tws_paymentrequest AS VARCHAR)              AS payment_request_id,
    trn.tws_paymentrequest                          AS payment_request_name,
    CAST(trn.tws_certificate AS VARCHAR)                 AS certificate_id,
    trn.tws_certificate                              AS certificate_name,
    CAST(trn.tws_employee_application AS VARCHAR)        AS employee_application_id,
    trn.tws_employee_application                    AS employee_application_name,
    CAST(emp.tws_employeeapplicationid AS VARCHAR)       AS employee_application_anchor_id,
    CAST(entapp.tws_enterpriseapplicationid AS VARCHAR)  AS enterprise_application_id,
    entapp.tws_name                                      AS enterprise_application_name,
    CAST(entapp.tws_maincompany AS VARCHAR)              AS company_id,
   ...
   trn.tws_training_provider                       AS training_provider,
    ...
    -- Workflow / status (decoded via option-sets)
     CASE WHEN trn.tws_payable_to IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('tws_trainingenrollment') || '|' || lower('tws_payable_to') || '|' || CAST(trn.tws_payable_to AS VARCHAR)) END AS payable_to, 
     CASE WHEN trn.tws_workflow_status IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('tws_trainingenrollment') || '|' || lower('tws_workflow_status') || '|' || CAST(trn.tws_workflow_status AS VARCHAR)) END AS workflow_status, 
     CASE WHEN trn.statuscode IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('tws_trainingenrollment') || '|' || lower('statuscode') || '|' || CAST(trn.statuscode AS VARCHAR)) END AS status_reason, 
     CASE WHEN trn.tws_supportpercentage IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('tws_trainingenrollment') || '|' || lower('tws_SupportPercentage') || '|' || CAST(trn.tws_supportpercentage AS VARCHAR)) END AS support_percentage, 
   ...
   com.tmkn_tamkeencompanycategory                 AS tamkeen_company_category,
    ...
    CAST(NULL AS VARCHAR) AS created_by_send_to_checker,
    CAST(NULL AS VARCHAR) AS created_by_send_to_manager,
    CAST(NULL AS VARCHAR) AS created_by_mark_approved,
    CAST(NULL AS VARCHAR) AS created_by_mark_disapproved,
    ...
    CAST(NULL AS VARCHAR) AS created_by_mark_withdrawn,
    CAST(NULL AS VARCHAR) AS created_by_mark_dropout,
...
FROM dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".TWS_TRAININGENROLLMENTBASE trn
INNER JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".TWS_EMPLOYEEAPPLICATIONBASE emp
...
INNER JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".TWS_ENTERPRISEAPPLICATIONBASE entapp
       ON entapp.tws_enterpriseapplicationid = emp.tws_enterprise_application
INNER JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".TMKN_COMPANYBASE com
       ON com.tmkn_companyid = entapp.tws_maincompany
INNER JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".MIS_CERTIFICATEBASE cert
       ON cert.mis_certificateid = trn.tws_certificate
INNER JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".MIS_CERTIFICATEEXPRIEBASE crtexp
...
os2_branch AS (
    ...
    SELECT
        ...
        source_system_name,
        CAST('application_support' AS VARCHAR)                          AS source_grain,

        profile_type                                                    AS profile_type,

        CAST(application_id AS VARCHAR)                                 AS training_record_id,
        ...
        CAST(NULL AS VARCHAR)                                           AS training_enrollment_id,
        CAST(NULL AS VARCHAR)                                           AS training_enrollment_name,
        ...
        CAST(NULL AS VARCHAR)                                           AS employee_application_anchor_id,
        ...
        CAST(NULL AS VARCHAR)                                            AS support_percentage,

        CAST(NULL AS VARCHAR)                                           AS certificate_approval,
        CAST(NULL AS VARCHAR)                                           AS created_by_partner,
        CAST(NULL AS VARCHAR)                                           AS checker_name,
        CAST(NULL AS VARCHAR)                                           AS monitoring_batch,
        CAST(NULL AS VARCHAR)                                           AS justification_for_choosing_cert,
        ...
        CAST(NULL AS VARCHAR)                                           AS commercial_name_english,
        ...
        CAST(NULL AS VARCHAR)                                           AS product_pid,
...
mis_branch AS (
    ...
    SELECT
        ...
        CAST('training_enrollment' AS VARCHAR)                          AS source_grain,

        CAST('ENT' AS VARCHAR)                                          AS profile_type,

        CAST(training_enrollment_id AS VARCHAR)                         AS training_record_id,
        ...
        CAST(NULL AS VARCHAR)                                           AS assessment_workflow_status,

        CAST(NULL AS VARCHAR)                                           AS support_decision,
        ...
        CAST(NULL AS VARCHAR)                                           AS employee_status,
...
bronze_normalized AS (
    SELECT
        ...
        CAST("source_grain" AS VARCHAR) AS "source_grain",
        CAST("profile_type" AS VARCHAR) AS "profile_type",
        CAST("training_record_id" AS VARCHAR) AS "training_record_id",
        CAST("application_id" AS VARCHAR) AS "application_id",
        ...
        CAST("assessment_workflow_status" AS VARCHAR) AS "assessment_workflow_status",
        CAST("support_decision" AS VARCHAR) AS "support_decision",
        ...
        CAST("approval_letter_accepted_on" AS VARCHAR) AS "approval_letter_accepted_on",
        CAST("customer_full_name" AS VARCHAR) AS "customer_full_name",
        CAST("cpr" AS VARCHAR) AS "cpr",
        CAST("employee_status" AS VARCHAR) AS "employee_status",
        ...
        CAST("training_start_date" AS VARCHAR) AS "training_start_date",
        CAST("training_end_date" AS VARCHAR) AS "training_end_date",
        CAST("training_assessment_date" AS VARCHAR) AS "training_assessment_date",
        CAST("certification_cap_amount_bhd" AS VARCHAR) AS "certification_cap_amount_bhd",
        CAST("tamkeen_cap_amount_bhd" AS VARCHAR) AS "tamkeen_cap_amount_bhd",
        CAST("tamkeen_share_amount_bhd" AS VARCHAR) AS "tamkeen_share_amount_bhd",
        CAST("tamkeen_share_pct" AS VARCHAR) AS "tamkeen_share_pct",
        CAST("customer_share_pct" AS VARCHAR) AS "customer_share_pct",
        CAST("customer_share_amount_with_vat" AS VARCHAR) AS "customer_share_amount_with_vat",
        CAST("unutilized_amount_bhd" AS VARCHAR) AS "unutilized_amount_bhd",
        CAST("total_vat_amount_bhd" AS VARCHAR) AS "total_vat_amount_bhd",
        CAST("effective_tamkeen_share_amount_bhd" AS VARCHAR) AS "effective_tamkeen_share_amount_bhd",
        ...
        CAST("training_enrollment_id" AS VARCHAR) AS "training_enrollment_id",
        CAST("training_enrollment_name" AS VARCHAR) AS "training_enrollment_name",
        ...
        CAST("employee_application_anchor_id" AS VARCHAR) AS "employee_application_anchor_id",
        ...
        CAST("proposed_finish_date" AS VARCHAR) AS "proposed_finish_date",
        CAST("employer_share" AS VARCHAR) AS "employer_share",
        CAST("discount" AS VARCHAR) AS "discount",
        CAST("total_expense" AS VARCHAR) AS "total_expense",
        CAST("total_travel_cap" AS VARCHAR) AS "total_travel_cap",
        CAST("payable_to" AS VARCHAR) AS "payable_to",
        CAST("status_reason" AS VARCHAR) AS "status_reason",
        CAST("support_percentage" AS VARCHAR) AS "support_percentage",
        CAST("certificate_approval" AS VARCHAR) AS "certificate_approval",
        CAST("created_by_partner" AS VARCHAR) AS "created_by_partner",
        CAST("checker_name" AS VARCHAR) AS "checker_name",
        CAST("monitoring_batch" AS VARCHAR) AS "monitoring_batch",
        CAST("justification_for_choosing_cert" AS VARCHAR) AS "justification_for_choosing_cert",
        CAST("product_pid_name" AS VARCHAR) AS "product_pid_name",
        CAST("individual_email" AS VARCHAR) AS "individual_email",
        CAST("individual_mobile" AS VARCHAR) AS "individual_mobile",
        CAST("individual_gender" AS VARCHAR) AS "individual_gender",
        CAST("individual_highest_degree" AS VARCHAR) AS "individual_highest_degree",
        CAST("individual_university_specialization" AS VARCHAR) AS "individual_university_specialization",
        ...
        CAST("certificate_broad" AS VARCHAR) AS "certificate_broad",
        CAST("certificate_detailed" AS VARCHAR) AS "certificate_detailed",
        CAST("certificate_narrow" AS VARCHAR) AS "certificate_narrow",
        CAST("certificate_type" AS VARCHAR) AS "certificate_type",
        CAST("certificate_cap" AS VARCHAR) AS "certificate_cap",
        CAST("awarding_body" AS VARCHAR) AS "awarding_body",
        CAST("training_duration_hours" AS VARCHAR) AS "training_duration_hours",
        CAST("product_pid" AS VARCHAR) AS "product_pid",
        CAST("last_submission_date" AS VARCHAR) AS "last_submission_date",
        CAST("created_by_send_to_checker" AS VARCHAR) AS "created_by_send_to_checker",
        CAST("created_by_send_to_manager" AS VARCHAR) AS "created_by_send_to_manager",
        CAST("created_by_mark_approved" AS VARCHAR) AS "created_by_mark_approved",
        CAST("created_by_mark_disapproved" AS VARCHAR) AS "created_by_mark_disapproved",
        CAST("last_disapproved_date" AS VARCHAR) AS "last_disapproved_date",
        CAST("created_by_mark_withdrawn" AS VARCHAR) AS "created_by_mark_withdrawn",
        CAST("created_by_mark_dropout" AS VARCHAR) AS "created_by_mark_dropout",
        CAST("last_started_date" AS VARCHAR) AS "last_started_date",
        CAST("last_finished_date" AS VARCHAR) AS "last_finished_date",
        CAST("last_mark_approved_by_employee_date" AS VARCHAR) AS "last_mark_approved_by_employee_date",
        CAST("last_mark_disapproved_by_employee_date" AS VARCHAR) AS "last_mark_disapproved_by_employee_date",
        CAST("last_mark_disapproved_due_parent_date" AS VARCHAR) AS "last_mark_disapproved_due_parent_date",
        CAST("first_send_back_to_maker_date" AS VARCHAR) AS "first_send_back_to_maker_date",
        CAST("last_send_back_to_maker_date" AS VARCHAR) AS "last_send_back_to_maker_date",
        CAST("total_count_send_back_to_maker" AS VARCHAR) AS "total_count_send_back_to_maker",
        CAST("first_send_back_to_portal_date" AS VARCHAR) AS "first_send_back_to_portal_date",
        CAST("last_send_back_to_portal_date" AS VARCHAR) AS "last_send_back_to_portal_date",
        CAST("total_count_send_back_to_portal" AS VARCHAR) AS "total_count_send_back_to_portal",
        CAST("last_send_to_tsp_date" AS VARCHAR) AS "last_send_to_tsp_date",
        CAST("last_tsp_accepted_date" AS VARCHAR) AS "last_tsp_accepted_date",
        CAST("last_tsp_rejected_date" AS VARCHAR) AS "last_tsp_rejected_date",
