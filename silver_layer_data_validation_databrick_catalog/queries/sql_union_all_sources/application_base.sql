-- Compare bronze-layer query output with silver-layer table output for application_base.
...
--
-- Bronze source: C:\Users\MODICHERLA\OneDrive - Hexalytics, Inc\Documents\Requirements\Silver Layer\Union All sources\updated_silver_layer_scripts\union of os2_os1_mis\application_base_os2_os1_mis_union_bronze_layer.sql
-- Silver source: C:\Users\MODICHERLA\OneDrive - Hexalytics, Inc\Documents\Requirements\Silver Layer\Union All sources\updated_silver_layer_scripts\silver_layer_query\application_base_silver_layer.sql
...
WITH CTE AS (
    ...
    FROM dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".OSUSR_NTP_APPLICATION
)
...
SELECT APP.id,
   ...
   --- APP.FINALAPPROVEDTKSHAREAMT                           AS final_amount_tamkeen_share,
        CAST (NULL AS VARCHAR) AS finalapprovedtkshareamt,
            CAST (NULL AS VARCHAR) AS final_amount_tamkeen_share,
...
============================================================================
Per-source intermediate Silver model for the Application domain Ã¢â‚¬â€ OS1 only.

Sources (Application domain):
  Ã¢Ëœâ€¦ OSUSR_PX1_APPLICATION                 Ã¢â€ â€™ anchor: the application entity
    OSUSR_PX1_APPLICATIONSUPPORTDETAILS   Ã¢â€ â€™ 1:1 wide attribute extension
                                              (RM/Assessor/Approver scoring,
                                              financing details, grant amounts Ã¢â‚¬â€
                                              ~40 columns)
    OSUSR_PX1_APPLICATIONSYB              Ã¢â€ â€™ 1:1 SYB-specific attributes
                                              (turnover, sector Ã¢â‚¬â€ used for
                                              Start-Your-Business applications)
    OSUSR_PX1_APPLICATIONINTERNALSTATUSUPDATES21 Ã¢â€ â€™ many:1 Ã¢â‚¬â€ internal status
                                              audit trail; here we pick the
                                              LATEST per application
    OSUSR_PX1_CUSTOMERTRAINING            Ã¢â€ â€™ 1:1 customer-training details
    ossys_User (Ãƒâ€”3)                       Ã¢â€ â€™ RM, Assessor, Approver people
...
Reference SPs:
  ...
  - RPT-156_neoTamkeen_Customer_Contact    (slice Ã¢â‚¬â€ application + user + ext)
...
Notes:
  ...
  - The 40 support-details columns make this file wide but they're all 1:1
    with the application Ã¢â‚¬â€ no duplication risk.
  - Internal status: the source SP uses a temp-table to find the latest update
    ...
    QUALIFY ROW_NUMBER() Ã¢â‚¬â€ same logic, set-based, idiomatic for Trino.
...
WITH latest_internal_status AS (
    ...
    FROM (
        ...
        FROM dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".OSUSR_PX1_APPLICATIONINTERNALSTATUSUPDATES
    ) ranked
    WHERE rn = 1
)
...
SELECT
    ...
    CAST(NULL AS VARCHAR) AS application_type,
    stus.LABEL                                                            AS application_status,
    --AR.LABEL                                                              AS assessor_recommendation,
    CAST(NULL AS VARCHAR) AS assessor_recommendation,
    CAST(NULL AS VARCHAR) AS financing_approved_loan_status,
    ...
    -- Customer contact (slice from RPT-156 Ã¢â‚¬â€ joined via app.USERID Ã¢â€ â€™ ossys_user)
...
--LEFT JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".OSUSR_PX1_APPLICANTTYPE                  typ        ON typ.ID        = app.ID

-- SYB / sector / turnover 
LEFT JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".OSUSR_PX1_APPLICATIONTYPE                 p          ON p.ID          = app.ID
...
--LEFT JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".OSUSR_PX1_ASSESSORRECOMENDATION          AR         ON AR.ID         = app.ASSESSORRECOMENDATION
...
LEFT JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".OSUSR_PX1_BANKNAMES                      bank       ON bank.ID       = detls.APPLICATIONID

-- Customer training
LEFT JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".OSUSR_PX1_CUSTOMERTRAINING               CusTrn     ON CusTrn.APPLICATIONID = app.ID
LEFT JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".OSUSR_PX1_MODEOFDELIVERY                 MofD       ON MofD.ID       = CusTrn.MODEOFDELIVERYID
LEFT JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".OSUSR_PX1_TRAININGPROGRAM                TrnPrg     ON TrnPrg.ID     = CusTrn.TRAININGPROGRAMID
LEFT JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".OSUSR_PX1_TRAININGPROGRAMTYPE            TrnPrgTyp  ON TrnPrgTyp.ID  = CusTrn.TRAININGPROGRAMTYPEID
LEFT JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".OSUSR_PX1_TYPEOFTRAININGPROVIDER         TypTrnProv ON TypTrnProv.ID = CusTrn.TRAININGPROVIDERTYPEID
...
LEFT JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".OSUSR_PX1_APPLICATIONINTERNALSTATUSES  Int_ST     ON Int_ST.ID     = lis.internal_status_id

-- Customer (the applying user) Ã¢â‚¬â€ folded in from RPT-156
LEFT JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".OSSYS_USER                               Cust       ON Cust.ID       = app.USERID
LEFT JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".OSUSR_MKZ_USEREXTENSION                  CustExt    ON CustExt.USERID = Cust.ID
),
mis_source AS (
...
SELECT
    ...
    -- Identifiers
    CAST(app.mis_individualapplicationid AS VARCHAR) AS application_id,
    app.mis_id                                       AS application_no,

    -- Foreign keys
    CAST(app.mis_individualid AS VARCHAR)            AS individual_id,
    CAST(app.mis_certificatename AS VARCHAR)         AS certificate_id,
    CAST(app.tmkn_pid AS VARCHAR)                    AS pid,
    CAST(app.mis_productid AS VARCHAR)               AS product_id,
    ...
    CASE WHEN app.statecode IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('MIS_individualapplication') || '|' || lower('statecode') || '|' || CAST(app.statecode AS VARCHAR)) END                  AS state,
    CASE WHEN app.statuscode IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('MIS_individualapplication') || '|' || lower('statuscode') || '|' || CAST(app.statuscode AS VARCHAR)) END               AS status_reason,
    CASE WHEN app.mis_hasnopayment IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('MIS_individualapplication') || '|' || lower('mis_hasnopayment') || '|' || CAST(app.mis_hasnopayment AS VARCHAR)) END    AS has_no_payment,
    CASE WHEN app.mis_pass IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('MIS_individualapplication') || '|' || lower('mis_pass') || '|' || CAST(app.mis_pass AS VARCHAR)) END                 AS pass,
    CASE WHEN app.mis_studytype IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('MIS_individualapplication') || '|' || lower('mis_studytype') || '|' || CAST(app.mis_studytype AS VARCHAR)) END          AS study_type,
    CASE WHEN app.mis_tpcs_workflowstatus IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('MIS_individualapplication') || '|' || lower('mis_tpcs_workflowstatus') || '|' || CAST(app.mis_tpcs_workflowstatus AS VARCHAR)) END  AS workflow_status,
    CASE WHEN app.tmkn_withdrawreason IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('MIS_individualapplication') || '|' || lower('tmkn_withdrawreason') || '|' || CAST(app.tmkn_withdrawreason AS VARCHAR)) END      AS withdraw_reason,
    CASE WHEN app.tmkn_violationstatus IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('MIS_individualapplication') || '|' || lower('tmkn_violationstatus') || '|' || CAST(app.tmkn_violationstatus AS VARCHAR)) END     AS violation_status,
    CASE WHEN app.tmkn_segment IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('MIS_individualapplication') || '|' || lower('tmkn_segment') || '|' || CAST(app.tmkn_segment AS VARCHAR)) END             AS segment,
    ...
    CASE WHEN ind.mis_schoollevel IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('MIS_individual') || '|' || lower('mis_schoollevel') || '|' || CAST(ind.mis_schoollevel AS VARCHAR)) END              AS individual_school_level,
    CASE WHEN ind.mis_unversitylevel IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('MIS_individual') || '|' || lower('mis_unversitylevel') || '|' || CAST(ind.mis_unversitylevel AS VARCHAR)) END           AS individual_university_level,
    CASE WHEN ind.mis_universityspecialization IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('MIS_individual') || '|' || lower('mis_universityspecialization') || '|' || CAST(ind.mis_universityspecialization AS VARCHAR)) END AS individual_university_specialization,
    CASE WHEN ind.tmkn_highest_degree_obtained IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('MIS_individual') || '|' || lower('tmkn_highest_degree_obtained') || '|' || CAST(ind.tmkn_highest_degree_obtained AS VARCHAR)) END AS individual_highest_degree,
    CASE WHEN ind.mis_gender IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('MIS_individual') || '|' || lower('mis_gender') || '|' || CAST(ind.mis_gender AS VARCHAR)) END                   AS individual_gender,
    CASE WHEN ind.mis_qualification IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('MIS_individual') || '|' || lower('mis_qualification') || '|' || CAST(ind.mis_qualification AS VARCHAR)) END            AS individual_qualification,
    CASE WHEN ind.mis_nationality IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('MIS_individual') || '|' || lower('MIS_Nationality') || '|' || CAST(ind.mis_nationality AS VARCHAR)) END              AS individual_nationality,
    CAST(cert.tmkn_id AS VARCHAR)                    AS certificate_external_id,
    ...
    CASE WHEN cert.mis_type IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('mis_certificate') || '|' || lower('mis_type') || '|' || CAST(cert.mis_type AS VARCHAR)) END              AS certificate_type,
    CASE WHEN cert.tmkn_certificatetype IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('mis_certificate') || '|' || lower('tmkn_CertificateType') || '|' || CAST(cert.tmkn_certificatetype AS VARCHAR)) END  AS certificate_category,
    ...
    CAST(NULL AS VARCHAR)                            AS cr_number,
    ...
    CAST(NULL AS VARCHAR)                            AS tmkn_area,             
    CAST(NULL AS VARCHAR)                            AS tmkn_crregistrationdate,   
    CAST(NULL AS VARCHAR)                            AS  tws_job_title,
    CAST(NULL AS VARCHAR)                            AS   tws_job,  
    CAST(NULL AS VARCHAR)        AS tmkn_maincr,
    CAST(NULL AS VARCHAR)        AS tmkn_industry,
    CAST(NULL AS VARCHAR)        AS tmkn_workflowstatus,
    CAST(NULL AS VARCHAR)        AS tmkn_isapproved,
    ...
    CAST(NULL AS VARCHAR)        AS tmkn_objectiverouting,
    CAST(NULL AS TIMESTAMP)      AS tmkn_contractstartdate,
    CAST(NULL AS TIMESTAMP)      AS tmkn_contractenddate,
    CAST(NULL AS VARCHAR)        AS tmkn_primaryname,
    CAST(NULL AS VARCHAR)        AS tmkn_primarymobilenumber,
    CAST(NULL AS VARCHAR)        AS tmkn_primaryemail,
    CAST(NULL AS VARCHAR)        AS tmkn_blocknumber,  
       CAST(NULL AS VARCHAR)    as tmkn_commercialname,
...
FROM dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".MIS_INDIVIDUALAPPLICATIONBASE app
INNER JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".MIS_INDIVIDUALBASE ind
       ON ind.mis_individualid = app.mis_individualid
LEFT JOIN  dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".MIS_CERTIFICATEBASE cert
       ON cert.mis_certificateid = app.mis_certificatename
LEFT JOIN  dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".TMKN_PIDBASE pid
       ON pid.tmkn_pidid = app.tmkn_pid
LEFT JOIN  dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".MIS_PRODUCTBASE prod
       ON prod.mis_productid = app.mis_productid

-- Status history milestones Ã¢â‚¬â€ one LEFT JOIN per status of interest
...
SELECT
    ...
    CAST(app.tmkn_applicationid AS VARCHAR)          AS application_id,
    app.tmkn_name                                    AS application_no,
    CAST(NULL AS VARCHAR)                            AS individual_id,
    CAST(NULL AS VARCHAR)                            AS certificate_id,
    CAST(app.tmkn_pid AS VARCHAR)                    AS pid,
    ...
    CAST(NULL AS VARCHAR)                            AS training_provider,
    CAST(NULL AS VARCHAR)                            AS sponsorship,
    CAST(NULL AS VARCHAR)                            AS certificate_approval,
    CAST(NULL AS VARCHAR)                            AS assessment_analyst,
...
--    CASE WHEN EXTRACT(YEAR FROM app.tmkn_enddate) > 1900 THEN app.tmkn_enddate END AS end_date,
    ...
    CASE WHEN app.statecode IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('tmkn_application') || '|' || lower('statecode') || '|' || CAST(app.statecode AS VARCHAR)) END  AS state,
    CASE WHEN app.statuscode IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('tmkn_application') || '|' || lower('statuscode') || '|' || CAST(app.statuscode AS VARCHAR)) END AS status_reason,
    CAST(NULL AS VARCHAR)                            AS has_no_payment,
    CAST(NULL AS VARCHAR)                            AS pass,
    CAST(NULL AS VARCHAR)                            AS study_type,
    CASE WHEN app.tmkn_workflowstatus IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('tmkn_application') || '|' || lower('tmkn_workflowstatus') || '|' || CAST(app.tmkn_workflowstatus AS VARCHAR)) END   AS workflow_status,
    CAST(NULL AS VARCHAR)                            AS withdraw_reason,
    CAST(NULL AS VARCHAR)                            AS violation_status,
    CAST(NULL AS VARCHAR)                            AS segment,
    CAST(NULL AS VARCHAR)                            AS employment_details,
    CAST(NULL AS VARCHAR)                            AS justification_for_certification,
    app.modifiedby                                   AS modified_by,
    app.modifiedon                                   AS modified_on,
    ...
    CAST(NULL AS VARCHAR)                            AS individual_email,
    CAST(NULL AS VARCHAR)                            AS individual_mobile,
    CAST(NULL AS VARCHAR)                            AS individual_addr_flat,
    CAST(NULL AS VARCHAR)                            AS individual_addr_building,
    CAST(NULL AS VARCHAR)                            AS individual_addr_road,
    CAST(NULL AS VARCHAR)                            AS individual_addr_block,
    CAST(NULL AS VARCHAR)                            AS individual_addr_area,
    CAST(NULL AS VARCHAR)                            AS individual_school_level,
    CAST(NULL AS VARCHAR)                            AS individual_university_level,
    CAST(NULL AS VARCHAR)                            AS individual_university_specialization,
    CAST(NULL AS VARCHAR)                            AS individual_highest_degree,
    CAST(NULL AS VARCHAR)                            AS individual_gender,
    CAST(NULL AS VARCHAR)                            AS individual_qualification,
    CAST(NULL AS VARCHAR)                            AS individual_nationality,
    CAST(NULL AS VARCHAR)                            AS certificate_external_id,
    CAST(NULL AS VARCHAR)                            AS certificate_old_category,
    CAST(NULL AS VARCHAR)                            AS certificate_broad,
    CAST(NULL AS VARCHAR)                            AS certificate_detailed,
    CAST(NULL AS VARCHAR)                            AS certificate_narrow,
    CAST(NULL AS VARCHAR)                            AS certificate_awarding_body,
    CAST(NULL AS VARCHAR)                            AS certificate_type,
    CAST(NULL AS VARCHAR)                            AS certificate_category,
    ...
    sh_rejected.last_created_on                      AS last_rejected_date,
    sh_withdraw.last_created_on                      AS last_withdraw_date,
    CAST(app.tmkn_maincompany AS VARCHAR)            AS company_id,
    --app.tmkn_companyname                             AS company_name,
    CAST(NULL AS VARCHAR)                           AS company_name,  -- BD doesn't have company name at this layer; would need to join to account or similar
    CAST(NULL AS VARCHAR)                            AS cr_number,
    CAST(NULL AS VARCHAR)                            AS enterprise_application_id,
    CAST(NULL AS VARCHAR)                            AS employee_application_id,
    CAST(NULL AS DECIMAL(18, 2))                     AS approved_amount,
    CAST(NULL AS DECIMAL(18, 2))                     AS paid_amount,
    CAST(tmkn_area AS VARCHAR)                       AS tmkn_area,       
    CAST(tmkn_crregistrationdate AS VARCHAR)         AS tmkn_crregistrationdate,
    CAST(NULL AS VARCHAR)                            AS  tws_job_title,
    CAST(NULL AS VARCHAR)                            AS   tws_job,
    app.tmkn_maincr, --NEW
    CAST(app.tmkn_industry AS VARCHAR) AS tmkn_industry, --NEW
    CAST(app.tmkn_workflowstatus AS VARCHAR) as tmkn_workflowstatus, --NEW
    CAST(app.tmkn_isapproved AS VARCHAR) as tmkn_isapproved, --NEW
    ...
    CAST(app.tmkn_monitoringduedate AS timestamp(3)) as tmkn_monitoringduedate, --NEW
    CAST(app.tmkn_revenuey1 AS INTEGER) as tmkn_revenuey1, --NEW
    CAST(app.tmkn_objectiverouting AS VARCHAR) AS tmkn_objectiverouting, --NEW
...
FROM dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".TMKN_APPLICATIONBASE app
...
SELECT
    ...
    -- Identifiers
    CAST(emp.tws_employeeapplicationid AS VARCHAR)   AS application_id,
    emp.tws_name                                     AS application_no,

    -- Foreign keys
    --CAST(emp.tws_individualid AS VARCHAR)            AS individual_id,
    CAST(NULL AS VARCHAR)                            AS individual_id,  -- TWS Wage doesn't join to MIS_individual at this layer; would need to join to TWS Individual and then to MIS_individual
    CAST(NULL AS VARCHAR)                            AS certificate_id,
    CAST(NULL AS VARCHAR)                            AS pid,
  --  CAST(emp.tws_productid AS VARCHAR)               AS product_id,
    CAST(NULL AS VARCHAR)                            AS product_id, -- Product isn't a required field in TWS Wage, and the join to MIS_product causes significant performance issues due to the size of the MIS_product table. Omit product_id for now; can add back if needed with a more performant join strategy (e.g. pre-joining to a smaller subset of MIS_product or joining to a TWS Wage-specific product dimension if available)
    -- Display names
    --emp.tws_individualidname                         AS individual_name, 
    CAST(NULL AS VARCHAR)                            AS individual_name, -- TWS Wage doesn't join to MIS_individual at this layer; would need to join to TWS Individual and then to MIS_individual for the name
    CAST(NULL AS VARCHAR)                            AS certificate_name,
   ...
   -- emp.tws_tamkeenshare                             AS tamkeen_share,
    ...
    -- Status
    CASE WHEN emp.statecode IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('tws_employeeapplication') || '|' || lower('statecode') || '|' || CAST(emp.statecode AS VARCHAR)) END  AS state,
    CASE WHEN emp.statuscode IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('tws_employeeapplication') || '|' || lower('statuscode') || '|' || CAST(emp.statuscode AS VARCHAR)) END AS status_reason,
    CAST(NULL AS VARCHAR)                            AS has_no_payment,
    CAST(NULL AS VARCHAR)                            AS pass,
    CAST(NULL AS VARCHAR)                            AS study_type,
    ...
    -- Status history (TWS uses different SH table Ã¢â‚¬â€ left null until confirmed)
    ...
    CAST(ent.tws_enterpriseapplicationid AS VARCHAR) AS company_id,
    --ent.tws_companyname                              AS company_name,
    CAST(NULL AS VARCHAR)                            AS company_name,  -- TWS Wage doesn't have company name at this layer; would need to join to account or similar
    ent.tws_crnumber                                 AS cr_number,
    CAST(ent.tws_enterpriseapplicationid AS VARCHAR) AS enterprise_application_id,
    CAST(emp.tws_employeeapplicationid AS VARCHAR)   AS employee_application_id,
    CAST(NULL AS DECIMAL(18, 2))                     AS approved_amount,
    CAST(NULL AS DECIMAL(18, 2))                     AS paid_amount,
    CAST(NULL AS VARCHAR)                            AS tmkn_area, 
    CAST(NULL AS VARCHAR)                            AS tmkn_crregistrationdate,    
    CAST(TWS_JOB_TITLE AS VARCHAR),
    CAST(TWS_JOB AS VARCHAR),
    CAST(NULL AS VARCHAR)        AS tmkn_maincr,
    CAST(NULL AS VARCHAR)        AS tmkn_industry,
    CAST(NULL AS VARCHAR)        AS tmkn_workflowstatus,
    CAST(NULL AS VARCHAR)        AS tmkn_isapproved,
...
FROM dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".TWS_EMPLOYEEAPPLICATIONBASE emp
LEFT JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".TWS_ENTERPRISEAPPLICATIONBASE ent
...
os1 as (
    select
        ...
        -- OS2 COLUMNS
        -- =========================
        cast(null as varchar)     as reference_number,
        cast(null as varchar)     as program_type,
        cast(null as varchar)     as cr_license_cpr,
        ...
        cast(null as varchar)     as is_hipo_application,
        ...
        -- MIS COLUMNS
        -- =========================
        cast(null as varchar)     as application_subtype,
...
os2 as (
    ...
    select
        ...
        cast(null as varchar)     as turnover,
        program_name,
        cast(program_type as varchar) as program_type_name,
        ...
        cast(null as varchar)     as rm_remarks,
        ...
        cast(null as varchar)     as assessor_remarks,
        ...
        cast(null as varchar)     as approver_remarks,
        cast(null as decimal(18,2)) as approver_assessment_support_cap,
        cast(null as varchar)     as training_name_of_course_certification,
        cast(null as varchar)     as training_program_type,
        cast(null as varchar)     as training_program,
        cast(null as varchar)     as training_overview,
        cast(null as integer)     as training_cost,
        cast(null as varchar)     as training_mode_of_delivery,
        ...
        cast(null as varchar)     as training_provider_type,
        cast(null as varchar)     as training_provider_name,
        cast(null as varchar)     as training_awarding_body_name,
        cast(null as varchar)     as training_awarding_body_details,
        cast(null as varchar)     as training_country,
        cast(null as varchar)     as customer_name,
        cast(null as varchar)     as customer_mobile,
        cast(null as varchar)     as customer_email,
        cast(null as varchar)     as customer_nationality,
        cast(null as varchar)     as customer_gender,
        cast(null as timestamp)   as customer_date_of_birth,
        cast(null as varchar)     as customer_cpr_number,
...
SELECT
    ...
    reference_number,
    program_type
FROM dev_iceberg."tmkn-aws-dwh-dev-iceberg-silver".application_base
),
...
bronze_normalized AS (
    SELECT
        ...
        CAST("application_type_id" AS VARCHAR) AS "application_type_id",
        CAST("assessor_recommendation_id" AS VARCHAR) AS "assessor_recommendation_id",
        ...
        CAST("assessor_recommendation" AS VARCHAR) AS "assessor_recommendation",
        CAST("financing_approved_loan_status" AS VARCHAR) AS "financing_approved_loan_status",
        ...
        CAST("internal_status" AS VARCHAR) AS "internal_status",
        ...
        CAST("approve_pending_customer_confirmation_on" AS VARCHAR) AS "approve_pending_customer_confirmation_on",
        CAST("saved_on" AS VARCHAR) AS "saved_on",
        CAST("spending_period_end_date" AS VARCHAR) AS "spending_period_end_date",
        CAST("is_financing_approved" AS VARCHAR) AS "is_financing_approved",
        ...
        CAST("interest_rate" AS VARCHAR) AS "interest_rate",
        CAST("loan_start_date" AS VARCHAR) AS "loan_start_date",
        CAST("loan_end_date" AS VARCHAR) AS "loan_end_date",
        CAST("total_interest_amount" AS VARCHAR) AS "total_interest_amount",
        CAST("grace_period" AS VARCHAR) AS "grace_period",
        CAST("monthly_installment" AS VARCHAR) AS "monthly_installment",
        CAST("total_requested" AS VARCHAR) AS "total_requested",
        ...
        CAST("total_number_employees" AS VARCHAR) AS "total_number_employees",
        CAST("bank_name" AS VARCHAR) AS "bank_name",
        CAST("rm_name" AS VARCHAR) AS "rm_name",
        CAST("rm_remarks" AS VARCHAR) AS "rm_remarks",
        CAST("rm_score" AS VARCHAR) AS "rm_score",
        CAST("rm_recommended_financing" AS VARCHAR) AS "rm_recommended_financing",
        CAST("rm_recommended_grant" AS VARCHAR) AS "rm_recommended_grant",
        CAST("rm_assessment_support_cap" AS VARCHAR) AS "rm_assessment_support_cap",
        CAST("assessor_name" AS VARCHAR) AS "assessor_name",
        CAST("assessor_remarks" AS VARCHAR) AS "assessor_remarks",
        CAST("assessor_score" AS VARCHAR) AS "assessor_score",
        CAST("assessor_recommended_financing" AS VARCHAR) AS "assessor_recommended_financing",
        CAST("assessor_recommended_grant" AS VARCHAR) AS "assessor_recommended_grant",
        CAST("assessor_assessment_support_cap" AS VARCHAR) AS "assessor_assessment_support_cap",
        CAST("approver_name" AS VARCHAR) AS "approver_name",
        CAST("approver_remarks" AS VARCHAR) AS "approver_remarks",
        CAST("approver_assessment_support_cap" AS VARCHAR) AS "approver_assessment_support_cap",
        CAST("training_name_of_course_certification" AS VARCHAR) AS "training_name_of_course_certification",
        CAST("training_program_type" AS VARCHAR) AS "training_program_type",
        CAST("training_program" AS VARCHAR) AS "training_program",
        CAST("training_overview" AS VARCHAR) AS "training_overview",
        CAST("training_cost" AS VARCHAR) AS "training_cost",
        CAST("training_mode_of_delivery" AS VARCHAR) AS "training_mode_of_delivery",
        CAST("training_estimated_start_date" AS VARCHAR) AS "training_estimated_start_date",
        CAST("training_estimated_end_date" AS VARCHAR) AS "training_estimated_end_date",
        CAST("training_total_hours" AS VARCHAR) AS "training_total_hours",
        CAST("training_include_practical_hours" AS VARCHAR) AS "training_include_practical_hours",
        CAST("training_include_job_training" AS VARCHAR) AS "training_include_job_training",
        CAST("training_provider_type" AS VARCHAR) AS "training_provider_type",
        CAST("training_provider_name" AS VARCHAR) AS "training_provider_name",
        CAST("training_awarding_body_name" AS VARCHAR) AS "training_awarding_body_name",
        CAST("training_awarding_body_details" AS VARCHAR) AS "training_awarding_body_details",
        CAST("training_country" AS VARCHAR) AS "training_country",
        CAST("customer_name" AS VARCHAR) AS "customer_name",
        CAST("customer_mobile" AS VARCHAR) AS "customer_mobile",
        CAST("customer_email" AS VARCHAR) AS "customer_email",
        CAST("customer_nationality" AS VARCHAR) AS "customer_nationality",
        CAST("customer_gender" AS VARCHAR) AS "customer_gender",
        CAST("customer_date_of_birth" AS VARCHAR) AS "customer_date_of_birth",
        CAST("customer_cpr_number" AS VARCHAR) AS "customer_cpr_number",
        CAST("customer_cr_or_cpr" AS VARCHAR) AS "customer_cr_or_cpr",
        CAST("customer_application_name" AS VARCHAR) AS "customer_application_name",
