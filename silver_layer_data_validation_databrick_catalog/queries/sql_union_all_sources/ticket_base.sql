...
--
-- Bronze source: C:\Users\MODICHERLA\OneDrive - Hexalytics, Inc\Documents\Requirements\Silver Layer\Union All sources\updated_silver_layer_scripts\union of os2_os1_mis\ticket_base_os2_os1_mis_union_bronze_layer.sql
-- Silver source: C:\Users\MODICHERLA\OneDrive - Hexalytics, Inc\Documents\Requirements\Silver Layer\Union All sources\updated_silver_layer_scripts\silver_layer_query\ticket_base_silver_layer.sql
...
WITH ACTIVITY_CTE AS (
    ...
    SELECT
        ...
        CASE
            ...
            THEN NULL
            ELSE date_add('hour', 3, tik.CREATEDON)
        END AS RECEIVED_ON,

        CASE
            WHEN tik.CLOSEDON = TIMESTAMP '1900-01-01 00:00:00'
            THEN NULL
            ELSE date_add('hour', 3, tik.CLOSEDON)
        END AS ACTIONED_ON,
...
violation_types_agg AS (
    ...
    FROM dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".MIS_TMKN_VIOLATION_TMKN_VIOLATIONTYPESBASE vt
    INNER JOIN dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".TMKN_VIOLATIONTYPESBASE t
...
SELECT
    ...
    -- Identifiers
    CAST(viol.tmkn_violationid AS VARCHAR)               AS violation_id,
    ...
    CASE WHEN viol.tmkn_assignedviolationtype IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_assignedviolationtype') || '|' || CAST(viol.tmkn_assignedviolationtype AS VARCHAR)) END     AS assigned_violation_type,
    CASE WHEN viol.tmkn_suggestedviolationtype IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_suggestedviolationtype') || '|' || CAST(viol.tmkn_suggestedviolationtype AS VARCHAR)) END    AS suggested_violation_type,
    CASE WHEN viol.mis_suggestedviolationlevel IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('mis_suggestedviolationlevel') || '|' || CAST(viol.mis_suggestedviolationlevel AS VARCHAR)) END    AS suggested_violation_level,
    CASE WHEN viol.tmkn_category IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_category') || '|' || CAST(viol.tmkn_category AS VARCHAR)) END                  AS category,
    CASE WHEN viol.tmkn_scheme IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_scheme') || '|' || CAST(viol.tmkn_scheme AS VARCHAR)) END                    AS scheme,

    -- Status / workflow (decoded)
    CASE WHEN viol.tmkn_workflowstatus IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_workflowstatus') || '|' || CAST(viol.tmkn_workflowstatus AS VARCHAR)) END            AS workflow_status,
    CASE WHEN viol.tmkn_violationsworkflowstatuslist IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_violationsworkflowstatuslist') || '|' || CAST(viol.tmkn_violationsworkflowstatuslist AS VARCHAR)) END AS violations_workflow_status_list,
    CASE WHEN viol.statuscode IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('statuscode') || '|' || CAST(viol.statuscode AS VARCHAR)) END                     AS status_reason,
    CASE WHEN viol.statecode IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('statecode') || '|' || CAST(viol.statecode AS VARCHAR)) END                      AS state,

    -- Action option-sets (multi-action checkboxes from RPT-143)
    CASE WHEN viol.tmkn_sendwarningletter IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_sendwarningletter') || '|' || CAST(viol.tmkn_sendwarningletter AS VARCHAR)) END                AS action_send_warning_letter,
    CASE WHEN viol.tmkn_payorganizer IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_payorganizer') || '|' || CAST(viol.tmkn_payorganizer AS VARCHAR)) END                     AS action_pay_organizer,
    CASE WHEN viol.tmkn_sendtoeconomiccrimesdivision IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_sendtoeconomiccrimesdivision') || '|' || CAST(viol.tmkn_sendtoeconomiccrimesdivision AS VARCHAR)) END     AS action_send_to_ecd,
    CASE WHEN viol.tmkn_sendforsitevisitinspection IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_sendforsitevisitinspection') || '|' || CAST(viol.tmkn_sendforsitevisitinspection AS VARCHAR)) END       AS action_send_for_site_visit,
    CASE WHEN viol.tmkn_sendtocentralinformaticsorganisation IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_sendtocentralinformaticsorganisation') || '|' || CAST(viol.tmkn_sendtocentralinformaticsorganisation AS VARCHAR)) END AS action_send_to_cio,
    CASE WHEN viol.tmkn_addtoblacklist IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_addtoblacklist') || '|' || CAST(viol.tmkn_addtoblacklist AS VARCHAR)) END                   AS action_add_to_blacklist,
    CASE WHEN viol.tmkn_otherdecisions IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_otherdecisions') || '|' || CAST(viol.tmkn_otherdecisions AS VARCHAR)) END                   AS action_other_decisions,
    CASE WHEN viol.tmkn_batchcreated IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_batchcreated') || '|' || CAST(viol.tmkn_batchcreated AS VARCHAR)) END                     AS payment_batch_created,
    CASE WHEN viol.tmkn_supportbc IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_supportbc') || '|' || CAST(viol.tmkn_supportbc AS VARCHAR)) END                        AS action_support_bc,
    CASE WHEN viol.tmkn_encashguaranteecheque IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_encashguaranteecheque') || '|' || CAST(viol.tmkn_encashguaranteecheque AS VARCHAR)) END            AS action_encash_guarantee_cheque,
    viol.tmkn_otherdecisionstext                         AS other_decisions_text,

    -- Case description option-sets
    CASE WHEN viol.tmkn_bcviolation IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_bcviolation') || '|' || CAST(viol.tmkn_bcviolation AS VARCHAR)) END            AS case_desc_bc_violation,
    CASE WHEN viol.tmkn_forcemajeure IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_forcemajeure') || '|' || CAST(viol.tmkn_forcemajeure AS VARCHAR)) END           AS case_desc_force_majeure,
    CASE WHEN viol.tmkn_bcwithdrawal IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_bcwithdrawal') || '|' || CAST(viol.tmkn_bcwithdrawal AS VARCHAR)) END           AS case_desc_bc_withdrawal,
    CASE WHEN viol.tmkn_other IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_other') || '|' || CAST(viol.tmkn_other AS VARCHAR)) END                  AS case_desc_other,
    CASE WHEN viol.tmkn_cancellationofexhibition IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_cancellationofexhibition') || '|' || CAST(viol.tmkn_cancellationofexhibition AS VARCHAR)) END AS case_desc_cancellation_exhibition,
    CASE WHEN viol.tmkn_vendorviolation IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('TMKN_VIOLATIONBASE') || '|' || lower('tmkn_vendorviolation') || '|' || CAST(viol.tmkn_vendorviolation AS VARCHAR)) END        AS case_desc_vendor_violation,
...
FROM dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".TMKN_VIOLATIONBASE viol
...
cast (null as varchar) as violation_id,
cast (null as varchar) as violation_no,
cast (null as varchar) as reference_number,
cast (null as varchar) as cr_license_no,
cast (null as varchar) as violation_types,
cast (null as varchar) as assigned_violation_type,
cast (null as varchar) as suggested_violation_type,
cast (null as varchar) as suggested_violation_level,
cast (null as varchar) as category,
cast (null as varchar) as scheme,
cast (null as varchar) as workflow_status,
cast (null as varchar) as violations_workflow_status_list,
cast (null as varchar) as status_reason,
cast (null as varchar) as state,
cast (null as varchar) as action_send_warning_letter,
cast (null as varchar) as action_pay_organizer,
cast (null as varchar) as action_send_to_ecd,
cast (null as varchar) as action_send_for_site_visit,
cast (null as varchar) as action_send_to_cio,
cast (null as varchar) as action_add_to_blacklist,
cast (null as varchar) as action_other_decisions,
cast (null as varchar) as payment_batch_created,
cast (null as varchar) as action_support_bc,
cast (null as varchar) as action_encash_guarantee_cheque,
cast (null as varchar) as other_decisions_text,
cast (null as varchar) as case_desc_bc_violation,
cast (null as varchar) as case_desc_force_majeure,
cast (null as varchar) as case_desc_bc_withdrawal,
cast (null as varchar) as case_desc_other,
cast (null as varchar) as case_desc_cancellation_exhibition,
cast (null as varchar) as case_desc_vendor_violation,
...
cast (null as varchar) as follow_up_result,
cast (null as varchar) as manager_comment,
cast (null as varchar) as director_comment,
cast (null as varchar) as initiator_comments,
cast (null as varchar) as case_details,
cast (null as varchar) as tamkeens_decision,
cast (null as varchar) as consultant_justification_remarks,
cast (null as varchar) as grievance_decisions,
cast (null as varchar) as building,
cast (null as varchar) as road,
cast (null as varchar) as flat,
cast (null as varchar) as block,
cast (null as varchar) as area,
cast (null as varchar) as commercial_name,
cast (null as varchar) as mobile,
cast (null as varchar) as email,
cast (null as varchar) as representative_name,
cast (null as varchar) as supplier_name,
cast (null as varchar) as director_name,
cast (null as varchar) as manager_name,
cast (null as varchar) as officer_analyst_name,
cast (null as varchar) as consultant_name,
cast (null as varchar) as organizer_name,
cast (null as varchar) as contract_number_name,
cast (null as varchar) as service_provider_new_name,
cast (null as varchar) as service_provider_name,
cast (null as varchar) as site_visit_name,
cast (null as varchar) as monitoring_ticket_name,
cast (null as varchar) as application_no_name,
cast (null as varchar) as individual_name,
cast (null as varchar) as individual_application_name,
cast (null as varchar) as establishment_application_name,
cast (null as varchar) as bc_support_application_name,
cast (null as varchar) as bc_support_payment_name,
cast (null as varchar) as tws_enterprise_application_new_name,
cast (null as varchar) as tws_enterprise_application_name,
cast (null as varchar) as tws_employee_application_name,
cast (null as varchar) as exhibition_name,
cast (null as varchar) as from_department_name,
cast (null as varchar) as bds_payment_name,
cast (null as varchar) as e7trf_payment_name,
cast (null as varchar) as ict_payment_name,
cast (null as varchar) as gap_payment_name,
cast (null as varchar) as mas_payment_name,
cast (null as varchar) as qms_payment_name,
cast (null as varchar) as tap_payment_name,
cast (null as varchar) as es_payment_name_text,
cast (null as varchar) as es_payment_request_name,
cast (null as varchar) as bds_execution_record_name,
cast (null as varchar) as gap_execution_record_name,
cast (null as varchar) as ict_execution_record_name,
cast (null as varchar) as qms_execution_record_name,
cast (null as varchar) as mas_execution_record_name,
cast (null as varchar) as tap_execution_record_name,
cast (null as varchar) as cpp_client_name,
cast (null as varchar) as cpp_beneficiary_name,
cast (null as varchar) as cpp_pi_invoice_name,
cast (null as varchar) as cpp_tsp_name,
cast (null as varchar) as cpp_tsp_invoice_name,
cast (null as varchar) as fvr_member_name,
...
cast (null as varchar ) as refnumber,
cast (null as boolean ) as isactive,
cast (null as varchar ) as tkchannelid,
cast (null as varchar ) as tickettypeid,
cast (null as varchar ) as ticketstatusid,
...
bronze_normalized AS (
    SELECT
        ...
        CAST("entityidentifier" AS VARCHAR) AS "entityidentifier",
        CAST("refnumber" AS VARCHAR) AS "refnumber",
        CAST("isactive" AS VARCHAR) AS "isactive",
        CAST("tkchannelid" AS VARCHAR) AS "tkchannelid",
        CAST("tickettypeid" AS VARCHAR) AS "tickettypeid",
        CAST("ticketstatusid" AS VARCHAR) AS "ticketstatusid",
        CAST("status" AS VARCHAR) AS "status",
        CAST("type" AS VARCHAR) AS "type",
        CAST("received_on" AS VARCHAR) AS "received_on",
        CAST("actioned_on" AS VARCHAR) AS "actioned_on",
        CAST("average_resolution_time_hour" AS VARCHAR) AS "average_resolution_time_hour",
        CAST("average_resolution_time_days" AS VARCHAR) AS "average_resolution_time_days",
        ...
        CAST("violation_id" AS VARCHAR) AS "violation_id",
        CAST("violation_no" AS VARCHAR) AS "violation_no",
        CAST("reference_number" AS VARCHAR) AS "reference_number",
        CAST("cr_license_no" AS VARCHAR) AS "cr_license_no",
        CAST("violation_types" AS VARCHAR) AS "violation_types",
        CAST("assigned_violation_type" AS VARCHAR) AS "assigned_violation_type",
        CAST("suggested_violation_type" AS VARCHAR) AS "suggested_violation_type",
        CAST("suggested_violation_level" AS VARCHAR) AS "suggested_violation_level",
        CAST("category" AS VARCHAR) AS "category",
        CAST("scheme" AS VARCHAR) AS "scheme",
        CAST("workflow_status" AS VARCHAR) AS "workflow_status",
        CAST("violations_workflow_status_list" AS VARCHAR) AS "violations_workflow_status_list",
        CAST("status_reason" AS VARCHAR) AS "status_reason",
        CAST("state" AS VARCHAR) AS "state",
        CAST("action_send_warning_letter" AS VARCHAR) AS "action_send_warning_letter",
        CAST("action_pay_organizer" AS VARCHAR) AS "action_pay_organizer",
        CAST("action_send_to_ecd" AS VARCHAR) AS "action_send_to_ecd",
        CAST("action_send_for_site_visit" AS VARCHAR) AS "action_send_for_site_visit",
        CAST("action_send_to_cio" AS VARCHAR) AS "action_send_to_cio",
        CAST("action_add_to_blacklist" AS VARCHAR) AS "action_add_to_blacklist",
        CAST("action_other_decisions" AS VARCHAR) AS "action_other_decisions",
        CAST("payment_batch_created" AS VARCHAR) AS "payment_batch_created",
        CAST("action_support_bc" AS VARCHAR) AS "action_support_bc",
        CAST("action_encash_guarantee_cheque" AS VARCHAR) AS "action_encash_guarantee_cheque",
        CAST("other_decisions_text" AS VARCHAR) AS "other_decisions_text",
        CAST("case_desc_bc_violation" AS VARCHAR) AS "case_desc_bc_violation",
        CAST("case_desc_force_majeure" AS VARCHAR) AS "case_desc_force_majeure",
        CAST("case_desc_bc_withdrawal" AS VARCHAR) AS "case_desc_bc_withdrawal",
        CAST("case_desc_other" AS VARCHAR) AS "case_desc_other",
        CAST("case_desc_cancellation_exhibition" AS VARCHAR) AS "case_desc_cancellation_exhibition",
        CAST("case_desc_vendor_violation" AS VARCHAR) AS "case_desc_vendor_violation",
        CAST("violation_fraud_date" AS VARCHAR) AS "violation_fraud_date",
        CAST("contract_start_date" AS VARCHAR) AS "contract_start_date",
        CAST("contract_end_date" AS VARCHAR) AS "contract_end_date",
        CAST("amount_to_be_collected" AS VARCHAR) AS "amount_to_be_collected",
        CAST("collected_amount" AS VARCHAR) AS "collected_amount",
        CAST("collected_amount_state" AS VARCHAR) AS "collected_amount_state",
        CAST("collected_amount_last_updated_on" AS VARCHAR) AS "collected_amount_last_updated_on",
        CAST("balance" AS VARCHAR) AS "balance",
        CAST("tamkeen_share" AS VARCHAR) AS "tamkeen_share",
        CAST("bc_share" AS VARCHAR) AS "bc_share",
        CAST("total_contract_cost" AS VARCHAR) AS "total_contract_cost",
        CAST("exchange_rate" AS VARCHAR) AS "exchange_rate",
        CAST("follow_up_result" AS VARCHAR) AS "follow_up_result",
        CAST("manager_comment" AS VARCHAR) AS "manager_comment",
        CAST("director_comment" AS VARCHAR) AS "director_comment",
        CAST("initiator_comments" AS VARCHAR) AS "initiator_comments",
        CAST("case_details" AS VARCHAR) AS "case_details",
        CAST("tamkeens_decision" AS VARCHAR) AS "tamkeens_decision",
        CAST("consultant_justification_remarks" AS VARCHAR) AS "consultant_justification_remarks",
        CAST("grievance_decisions" AS VARCHAR) AS "grievance_decisions",
        CAST("building" AS VARCHAR) AS "building",
        CAST("road" AS VARCHAR) AS "road",
        CAST("flat" AS VARCHAR) AS "flat",
        ...
        CAST("officer_analyst_name" AS VARCHAR) AS "officer_analyst_name",
        CAST("consultant_name" AS VARCHAR) AS "consultant_name",
        CAST("organizer_name" AS VARCHAR) AS "organizer_name",
        CAST("contract_number_name" AS VARCHAR) AS "contract_number_name",
        CAST("service_provider_new_name" AS VARCHAR) AS "service_provider_new_name",
        CAST("service_provider_name" AS VARCHAR) AS "service_provider_name",
        CAST("site_visit_name" AS VARCHAR) AS "site_visit_name",
        CAST("monitoring_ticket_name" AS VARCHAR) AS "monitoring_ticket_name",
        CAST("application_no_name" AS VARCHAR) AS "application_no_name",
        CAST("individual_name" AS VARCHAR) AS "individual_name",
        CAST("individual_application_name" AS VARCHAR) AS "individual_application_name",
        CAST("establishment_application_name" AS VARCHAR) AS "establishment_application_name",
        CAST("bc_support_application_name" AS VARCHAR) AS "bc_support_application_name",
        CAST("bc_support_payment_name" AS VARCHAR) AS "bc_support_payment_name",
        CAST("tws_enterprise_application_new_name" AS VARCHAR) AS "tws_enterprise_application_new_name",
        CAST("tws_enterprise_application_name" AS VARCHAR) AS "tws_enterprise_application_name",
        CAST("tws_employee_application_name" AS VARCHAR) AS "tws_employee_application_name",
        CAST("exhibition_name" AS VARCHAR) AS "exhibition_name",
        CAST("from_department_name" AS VARCHAR) AS "from_department_name",
        CAST("bds_payment_name" AS VARCHAR) AS "bds_payment_name",
        CAST("e7trf_payment_name" AS VARCHAR) AS "e7trf_payment_name",
        CAST("ict_payment_name" AS VARCHAR) AS "ict_payment_name",
        CAST("gap_payment_name" AS VARCHAR) AS "gap_payment_name",
        CAST("mas_payment_name" AS VARCHAR) AS "mas_payment_name",
        CAST("qms_payment_name" AS VARCHAR) AS "qms_payment_name",
        CAST("tap_payment_name" AS VARCHAR) AS "tap_payment_name",
        CAST("es_payment_name_text" AS VARCHAR) AS "es_payment_name_text",
        CAST("es_payment_request_name" AS VARCHAR) AS "es_payment_request_name",
        CAST("bds_execution_record_name" AS VARCHAR) AS "bds_execution_record_name",
        CAST("gap_execution_record_name" AS VARCHAR) AS "gap_execution_record_name",
        CAST("ict_execution_record_name" AS VARCHAR) AS "ict_execution_record_name",
        CAST("qms_execution_record_name" AS VARCHAR) AS "qms_execution_record_name",
        CAST("mas_execution_record_name" AS VARCHAR) AS "mas_execution_record_name",
        CAST("tap_execution_record_name" AS VARCHAR) AS "tap_execution_record_name",
        CAST("cpp_client_name" AS VARCHAR) AS "cpp_client_name",
        CAST("cpp_beneficiary_name" AS VARCHAR) AS "cpp_beneficiary_name",
        CAST("cpp_pi_invoice_name" AS VARCHAR) AS "cpp_pi_invoice_name",
        CAST("cpp_tsp_name" AS VARCHAR) AS "cpp_tsp_name",
        CAST("cpp_tsp_invoice_name" AS VARCHAR) AS "cpp_tsp_invoice_name",
        CAST("fvr_member_name" AS VARCHAR) AS "fvr_member_name",
