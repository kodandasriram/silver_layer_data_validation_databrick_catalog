-- Compare bronze-layer Odoo calendar events with silver-layer odoo_calendar_event_base.
...
silver_layer AS (
    SELECT
        ...
        source_system_name,
        dbt_updated_at
    FROM dev_iceberg."tmkn-aws-dwh-dev-iceberg-silver".odoo_calendar_event_base
),
...
bronze_normalized AS (
    SELECT
        CAST(meeting_subject AS VARCHAR) AS meeting_subject,
        CAST(start_datetime AS VARCHAR) AS start_datetime,
        CAST(end_datetime AS VARCHAR) AS end_datetime,
        CAST(attendees AS VARCHAR) AS attendees,
        CAST(sector_team AS VARCHAR) AS sector_team,
        CAST(location AS VARCHAR) AS location,
        CAST(duration_hours AS VARCHAR) AS duration_hours,
        CAST(company_name AS VARCHAR) AS company_name,
        CAST(program_feedback_notes AS VARCHAR) AS program_feedback_notes,
