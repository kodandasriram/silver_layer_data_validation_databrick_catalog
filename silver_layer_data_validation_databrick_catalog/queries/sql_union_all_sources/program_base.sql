...
--
-- Bronze source: C:\Users\MODICHERLA\OneDrive - Hexalytics, Inc\Documents\Requirements\Silver Layer\Union All sources\updated_silver_layer_scripts\union of os2_os1_mis\program_base_os2_os1_mis_union_bronze_layer.sql
-- Silver source: C:\Users\MODICHERLA\OneDrive - Hexalytics, Inc\Documents\Requirements\Silver Layer\Union All sources\updated_silver_layer_scripts\silver_layer_query\program_base_silver_layer.sql
-- NOTE: Generated with only the 23 common columns for data mismatch counts because source column names/order differ.
...
SELECT
    ...
    -- Identifiers
    CAST(prod.mis_productid AS VARCHAR)                  AS program_id,
    prod.mis_name                                        AS program_name,

    -- Placeholders for fields specific to tmkn_tapproduct branch
    CAST(NULL AS VARCHAR)                                AS tap_program_code,
    CAST(NULL AS VARCHAR)                                AS tap_program_description,
    CAST(NULL AS VARCHAR)                                AS tap_program_status,
...
FROM dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".MIS_PRODUCTBASE prod
...
SELECT
    ...
    -- Identifiers
    CAST(tap.tmkn_tapproductid AS VARCHAR)               AS program_id,
    ...
    CASE WHEN tap.statuscode IS NULL THEN NULL ELSE element_at((SELECT option_values FROM option_set_map), lower('tmkn_tapproduct') || '|' || lower('statuscode') || '|' || CAST(tap.statuscode AS VARCHAR)) END  AS tap_program_status,
...
FROM dev_iceberg."tmkn-aws-dwh-dev-iceberg-bronze".TMKN_TAPPRODUCTBASE tap
)
SELECT
    source_system_name,
    CAST(id AS VARCHAR) AS program_id,
...
SELECT
    ...
    CAST(NULL AS VARCHAR) AS program_group,
    CAST(NULL AS BIGINT) AS program_version_id,
    CAST(NULL AS VARCHAR) AS customer_type,
    CAST(NULL AS VARCHAR) AS profile_type,
    CAST(NULL AS VARCHAR) AS reference,
    CAST(NULL AS VARCHAR) AS initials,
    CAST(NULL AS VARCHAR) AS description,
    CAST(NULL AS BOOLEAN) AS is_special_program,
    CAST(NULL AS VARCHAR) AS cancel_reason,
    CAST(NULL AS TIMESTAMP(6)) AS active_date,
    CAST(NULL AS TIMESTAMP(6)) AS end_date,
    CAST(NULL AS VARCHAR) AS cms_program_en,
    CAST(NULL AS VARCHAR) AS cms_program_ar,
...
bronze_normalized AS (
    SELECT
        ...
        CAST("tap_program_code" AS VARCHAR) AS "tap_program_code",
        CAST("tap_program_description" AS VARCHAR) AS "tap_program_description",
        CAST("program_status" AS VARCHAR) AS "program_status",
        CAST("program_group" AS VARCHAR) AS "program_group",
        CAST("program_version_id" AS VARCHAR) AS "program_version_id",
        ...
        CAST("initials" AS VARCHAR) AS "initials",
        CAST("description" AS VARCHAR) AS "description",
        CAST("is_special_program" AS VARCHAR) AS "is_special_program",
        ...
        CAST("is_show_interest_enabled" AS VARCHAR) AS "is_show_interest_enabled",
        CAST("program_minor_version_id" AS VARCHAR) AS "program_minor_version_id",
