...
--
-- Bronze source: C:\Users\MODICHERLA\OneDrive - Hexalytics, Inc\Documents\Requirements\Silver Layer\Union All sources\Silver_layer_03-June-2026\converted db script to databricks\Databricks_union of all sources\product_services_base.sql
-- Silver source: converted db script to databricks\silver_layer scripts\product_services_base_silver_layer.sql

WITH
bronze_layer AS (
/*
Generated Databricks union layer for product_services_base.
Column order and typed NULL placeholders follow dbt model: product_services_base.sql.
...
Source Tables : product_and_services_base_os2
                product_services_base_mis

Target Table : PRODUCT_SERVICES_BASE
...
--------------------------------------------------------------
1.0     | 2026-05-13 | Venkatesh  | Initial version
...
WITH
    product_and_services_base_os2 AS (
/* =================================================================================================
...
FINAL_DATA AS (
    ...
    app.submittedon + INTERVAL 3 HOURS AS submitted_on_application,

    amdment.submittedon + INTERVAL 3 HOURS AS submitted_on_amendment,

    CASE
        WHEN app_sta.label = 'Active'
            THEN app.approvedon + INTERVAL 3 HOURS
        ELSE NULL
        ...
        ELSE
            CASE
                ...
                WHEN asses_amed.closed <> TIMESTAMP '1900-01-01 00:00:00'
                    THEN asses_amed.closed + INTERVAL 3 HOURS
                ELSE amdment.approvedon + INTERVAL 3 HOURS
            END
    ...
    appsup.createdon + INTERVAL 3 HOURS AS created_on_application_support,

    ps.createdon + INTERVAL 3 HOURS AS created_on_item,

    ps.updatedon + INTERVAL 3 HOURS AS updated_on_item,
...
INNER JOIN `tmkn-dwh-iceberg-dev-fc`.`tmkn-aws-dwh-dev-iceberg-bronze`.`OSUSR_NTP_APPLICATION4` app
...
SELECT DISTINCT
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSBASE')

      AND LOWER(sm.attributename) = LOWER('tmkn_amendmentstatus')

      AND CAST(sm.attributevalue AS STRING) = CAST(item.tmkn_amendmentstatus AS STRING)
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSBASE')
      ...
      AND LOWER(sm.attributename) = LOWER('tmkn_bcsharepaymentmethod')

      AND CAST(sm.attributevalue AS STRING) = CAST(item.tmkn_bcsharepaymentmethod AS STRING)

) AS bc_share_payment_method,
    (
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSBASE')
      ...
      AND LOWER(sm.attributename) = LOWER('tmkn_condition')

      AND CAST(sm.attributevalue AS STRING) = CAST(item.tmkn_condition AS STRING)
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSBASE')
      ...
      AND LOWER(sm.attributename) = LOWER('tmkn_discount_type')

      AND CAST(sm.attributevalue AS STRING) = CAST(item.tmkn_discount_type AS STRING)
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSBASE')
      ...
      AND LOWER(sm.attributename) = LOWER('tmkn_functionality')

      AND CAST(sm.attributevalue AS STRING) = CAST(item.tmkn_functionality AS STRING)
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSBASE')
      ...
      AND LOWER(sm.attributename) = LOWER('tmkn_itemstatus')

      AND CAST(sm.attributevalue AS STRING) = CAST(item.tmkn_itemstatus AS STRING)
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSBASE')
      ...
      AND LOWER(sm.attributename) = LOWER('tmkn_monitoringfinished')

      AND CAST(sm.attributevalue AS STRING) = CAST(item.tmkn_monitoringfinished AS STRING)

)  AS monitoring_finished,
    (
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSBASE')
      ...
      AND LOWER(sm.attributename) = LOWER('tmkn_payableto')

      AND CAST(sm.attributevalue AS STRING) = CAST(item.tmkn_payableto AS STRING)
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSBASE')
      ...
      AND LOWER(sm.attributename) = LOWER('tmkn_paymentshare')

      AND CAST(sm.attributevalue AS STRING) = CAST(item.tmkn_paymentshare AS STRING)
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSBASE')
      ...
      AND LOWER(sm.attributename) = LOWER('tmkn_servicetype')

      AND CAST(sm.attributevalue AS STRING) = CAST(item.tmkn_servicetype AS STRING)
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSBASE')
      ...
      AND LOWER(sm.attributename) = LOWER('tmkn_sitevisitstatus')

      AND CAST(sm.attributevalue AS STRING) = CAST(item.tmkn_sitevisitstatus AS STRING)

)     AS site_visit_status,
    (
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSBASE')
      ...
      AND CAST(sm.attributevalue AS STRING) = CAST(item.statecode AS STRING)
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSBASE')
      ...
      AND LOWER(sm.attributename) = LOWER('tmkn_uploadcompleted')

      AND CAST(sm.attributevalue AS STRING) = CAST(item.tmkn_uploadcompleted AS STRING)
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSBASE')
      ...
      AND LOWER(sm.attributename) = LOWER('tmkn_vatapplicable')

      AND CAST(sm.attributevalue AS STRING) = CAST(item.tmkn_vatapplicable AS STRING)
...
SELECT DISTINCT
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ITEMREQUESTBASE')

      AND LOWER(sm.attributename) = LOWER('statecode')

      AND CAST(sm.attributevalue AS STRING) = CAST(req.statecode AS STRING)
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ITEMREQUESTBASE')
      ...
      AND LOWER(sm.attributename) = LOWER('tmkn_isinitialrequest')

      AND CAST(sm.attributevalue AS STRING) = CAST(req.tmkn_isinitialrequest AS STRING)

) AS is_initial_request,
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ITEMREQUESTBASE')
      ...
      AND CAST(sm.attributevalue AS STRING) = CAST(req.tmkn_workflowstatus AS STRING)
...
SELECT DISTINCT
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSSPECIALCONDITIONBASE')

      AND LOWER(sm.attributename) = LOWER('tmkn_condition')

      AND CAST(sm.attributevalue AS STRING) = CAST(escon.tmkn_condition AS STRING)
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSSPECIALCONDITIONBASE')
      ...
      AND CAST(sm.attributevalue AS STRING) = CAST(escon.statuscode AS STRING)
    ...
    WHERE LOWER(ev.name) = LOWER('TMKN_ESITEMSSPECIALCONDITIONBASE')
      ...
      AND CAST(sm.attributevalue AS STRING) = CAST(escon.tmkn_workflowstatus AS STRING)

) AS special_condition_workflow_status,
...
from  product_and_services_base_os2
...
select
   --common
    ...
    report_date,
    dbt_updated_at
from  product_services_base_mis
),
...
bronze_normalized AS (
    SELECT
        ...
        CAST(`item_name_2` AS STRING) AS `item_name_2`,
